...
test_that("Subset assignment of hux into hux preserves attributes", {
  ...
  expect_equal(font(ht), matrix(c(rep(NA, 4), rep("italic", 2), NA, rep("italic", 2)), 3, 3), ignore_attr = TRUE)

  ht3 <- hux(1, 1, 1)
  row_height(ht3) <- "40px"
  ht[1, ] <- ht3
  expect_equal(row_height(ht)[1], "40px", ignore_attr = TRUE)

  ht4 <- hux(1:3)
  col_width(ht4) <- "20px"
  ht[, 2] <- ht4
  expect_equal(col_width(ht)[2], "20px", ignore_attr = TRUE)

  ht5 <- hux(a = 1:3, b = 1:3, d = 1:3, add_colnames = FALSE)
  font(ht5) <- "times"
  expect_silent(ht[] <- ht5)
  expect_equal(font(ht), matrix("times", 3, 3), ignore_attr = TRUE)

  ht7 <- hux(1:2, 1:2, 1:2)
  bold(ht7)[1, ] <- TRUE
  expect_silent(ht[1:2, ] <- ht7) # assignment of a non-square matrix
  expect_equal(bold(ht)[1, ], rep(TRUE, 3), ignore_attr = TRUE)
  expect_equal(bold(ht)[2, ], rep(FALSE, 3), ignore_attr = TRUE)
})
...
test_that("Can delete columns from a huxtable by setting it to `NULL`", {
  ...
  expect_equal(font(ht), matrix(NA_character_, 2, 1), ignore_attr = TRUE)
  expect_equal(col_width(ht), NA_real_, ignore_attr = TRUE)

  ht2 <- hux(a = 1:2, b = 1:2, add_colnames = FALSE)
  expect_silent(ht2[["a"]] <- NULL)
  expect_equal(font(ht2), matrix(NA_character_, 2, 1), ignore_attr = TRUE)
  expect_equal(col_width(ht2), NA_real_, ignore_attr = TRUE)

  ht3 <- hux(a = 1:2, b = 1:2, add_colnames = FALSE)
  expect_silent(ht3["a"] <- NULL)
  expect_equal(font(ht3), matrix(NA_character_, 2, 1), ignore_attr = TRUE)
  expect_equal(col_width(ht3), NA_real_, ignore_attr = TRUE)
  ...
  if (getRversion() >= "3.3.3") {
    ...
    expect_equal(font(ht4), matrix(NA_character_, 2, 1), ignore_attr = TRUE)
    expect_equal(col_width(ht4), NA_real_, ignore_attr = TRUE)

    ht5 <- hux(a = 1:2, b = 1:2, c = 1:2, add_colnames = FALSE)
    expect_silent(ht5[, c("a", "b")] <- NULL)
    expect_equal(font(ht5), matrix(NA_character_, 2, 1), ignore_attr = TRUE)
    expect_equal(col_width(ht5), NA_real_, ignore_attr = TRUE)
  }
})
...
test_that("Add row(s) by standard replacement methods", {
  ...
  expect_equal(font(ht), matrix(NA_character_, 3, 2), ignore_attr = TRUE)
  expect_equal(row_height(ht), rep(NA_real_, 3), ignore_attr = TRUE)
