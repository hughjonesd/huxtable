...
test_that("style_cells works with border properties", {
  ...
  expect_equal(brdr_thickness(bottom_border(ht)), matrix(0.4, 2, 2), ignore_attr = TRUE)

  expect_silent(ht <- style_cells(ht, bottom_border = brdr(0.6)))
  expect_equal(brdr_thickness(bottom_border(ht)), matrix(0.6, 2, 2), ignore_attr = TRUE)

  expect_silent(ht <- style_cells(ht, top_border_color = "red"))
  expect_equal(top_border_color(ht), matrix("red", 2, 2), ignore_attr = TRUE)
})
...
test_that("style_headers et al.", {
  ...
  expect_equal(bold(ht), matrix(c(TRUE, FALSE), 2, 1), ignore_attr = TRUE)
  ...
  expect_equal(bold(ht), matrix(c(TRUE, FALSE), 1, 2), ignore_attr = TRUE)
})
