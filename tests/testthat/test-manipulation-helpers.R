...
test_that("add_footnote works", {
  ...
  expect_equivalent(nrow(ht_orig), 4)  # header + 2 data + 1 footnote
  expect_equivalent(colspan(ht_orig)[4, 1], ncol(ht_orig))  # footnote is in row 4
...
test_that("insert_column and insert_row work", {
  ...
  expect_equivalent(nrow(ht), 4)  # header + 2 data + 1 inserted row
  expect_equivalent(ht[1, 2], "9")  # Inserted at top, column 2

  ht <- insert_row(ht_orig, 8, 9, after = 1)
  expect_equivalent(nrow(ht), 4)  # header + 1 data + 1 inserted + 1 data
  expect_equivalent(ht[[2, 2]], "9")  # Check individual values
  expect_equivalent(ht[[3, 2]], "1")
  expect_equivalent(ht[[4, 2]], "2")

  ht <- insert_row(ht_orig, 1, fill = 0, after = 1)
  expect_equivalent(ht[2, ], huxtable("1", "0", add_colnames = FALSE))

  ht <- insert_column(ht_orig, c("X", "8", "9"))  # Header + data for all 3 rows
  expect_equivalent(ncol(ht), 3)
  expect_equivalent(ht[2, 1], "8")  # First data row, first column

  ht <- insert_column(ht_orig, c("X", "8", "9"), after = 1)
  expect_equivalent(ncol(ht), 3)
  expect_equivalent(ht[1, 1], "a")  # Check individual values instead
  expect_equivalent(ht[1, 2], "X")  # Check individual values instead
  expect_equivalent(ht[1, 3], "b")  # Check individual values instead

  ht <- insert_column(ht_orig, c("Y", "3", "0"), fill = 0)
  expect_equivalent(ht[[2, 1]], "3")  # Check individual values
  expect_equivalent(ht[[3, 1]], "0")

  ht <- insert_column(ht_orig, "3", fill = 0, rowspan = 2)
  expect_equivalent(rowspan(ht)[1:2, 1], c(2, 1))  # Header and first data row
...
test_that("insert_column works with column names", {
  ...
  expect_equivalent(ncol(ht), 3)
  expect_equivalent(ht[[2, 2]], "8")  # Check individual values
  expect_equivalent(ht[[3, 2]], "9")
})
