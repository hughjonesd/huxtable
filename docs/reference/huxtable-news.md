...
- Bugfix: [`add_rownames()`](add_colnames.md) should no longer add a 1
  in the header row of a data frame.

## huxtable 5.8.0

### Other changes

- Redesigned [`hux_logo()`](hux_logo.md) function to spell out
  “huxtable” with customizable layout, fonts, and colors. Added
...
- Added Typst export via [`to_typst()`](to_typst.md) and
  [`print_typst()`](to_typst.md). Quarto integration is available as
  well as [`quick_typst()`](quick-output.md),
  [`quick_typst_pdf()`](quick-output.md),
  [`quick_typst_png()`](quick-output.md), and
  [`quick_typst_svg()`](quick-output.md) functions.

- HTML output now uses CSS classes with a shared `<style>` block instead
  of long inline styles.

- Added [`as_html()`](to_html.md) for obtaining table as `htmltools`
  tags.

- [`to_screen()`](to_screen.md) now displays double, dashed and dotted
  border styles.
...
- You can now add multiple huxtables to the same Excel worksheet in
  [`as_Workbook()`](as_Workbook.md). Suggestion by @oobd.
...
- Bugfix: [`by_cases()`](by_cases.md) wasn’t picking up variables from
...
- Update [`by_cases()`](by_cases.md) to work with dplyr 1.1.0. Within
  [`by_cases()`](by_cases.md) formulas, `.` is now vector rather than
  matrix when dplyr version 1.1.0 is detected. Thanks @DavisVaughan.
...
- New [`column_to_header()`](column_to_header.md) function converts a
  column to header rows. New [`as_hux()`](as_huxtable.md) method for
  `grouped_df` objects optionally converts groups to header rows.

- New convenience functions [`stripe_rows()`](stripes.md) and
  [`stripe_columns()`](stripes.md).

- Add `format` and `...` options to [`fmt_percent()`](fmt_percent.md) to
  allow flexible formatting via
  [`formatC()`](https://rdrr.io/r/base/formatc.html).

- [`add_footnote()`](add_footnote.md) gets an explicit `number_format`
  argument which is `NA` by default.

- Bugfix: infinite loop with wide characters in
  [`to_screen()`](to_screen.md).
...
- New behaviour: setting [`colspan()`](spans.md) or
  [`rowspan()`](spans.md) overwrites the content of cells that have been
  shadowed.
...
- Bugfix: centre alignment was not working in
  [`print_screen()`](to_screen.md).

- Bugfix: failure in [`to_md()`](to_md.md) with recent versions of
  `stringi` package.
...
- Allow [`tidy_override()`](tidy_override.md) to extend columns of
  `tidy` and `glance`.
...
- [`as_flextable()`](as_flextable.md) now exports markdown in cells to
  RTF, and to Word with the help of the optional `ftExtra` package.
  Thanks @atusy for adding this feature.

- Improvements to markdown screen export. This now uses the optional
  `fansi` package.

- New feature: [`as_Workbook()`](as_Workbook.md) gains `start_row` and
  `start_col` arguments, to write a huxtable into an Excel worksheet
  starting at a particular row or column.

- New feature: [`huxreg()`](huxreg.md) gains a `glance_args` argument to
  pass arguments to
...
- Bugfix: number format was not set correctly in
  [`huxreg()`](huxreg.md)’s `note`.

- Bugfix: in [`huxreg()`](huxreg.md), `tidy_args` threw an error if the
  first argument to
  [`tidy()`](https://generics.r-lib.org/reference/tidy.html) was a named
  list.

- Bugfix: [`tidy_replace()`](tidy_override.md) was broken.

- Clearer error messages for [`tidy_override()`](tidy_override.md) when
...
- There are changes to LaTeX output.
  ...
  - the `\centerbox` macro from the LaTeX “adjustbox” package is used to
    ...
    recent version.
    [`check_latex_dependencies()`](report_latex_dependencies.md) can
    inform you about this.

- As previously signalled, `add_colnames` has now become `TRUE` by
  default in [`huxtable()`](huxtable.md) and
  [`as_huxtable()`](as_huxtable.md). Set
...
- Huxtable borders have been reworked, fixing some longstanding bugs and
  ...
  - [`set_left_border()`](borders.md),
    [`set_all_borders()`](set-multiple.md) and friends all use a default
    value of 0.4. So to set a default border, write e.g.

        as_hux(head(iris)) %>%
              set_bottom_border(1, everywhere)

  - A new [`brdr()`](brdr.md) class encapsulates border thickness, style
...
- By default, [`width()`](width.md) is now unset.

- By default, [`wrap()`](wrap.md) is now `TRUE`.

- [`every()`](stripe.md) has been renamed to [`stripe()`](stripe.md), to
  avoid a clash with
...
- [`add_rows()`](add_rows.md) and [`add_columns()`](add_rows.md) now
  always call [`rbind.huxtable()`](cbind.huxtable.md) or
  [`cbind.huxtable()`](cbind.huxtable.md) and return a huxtable.

- Huxtable no longer supports dplyr versions less than 0.7.0 (released
  mid-2017).

- [`set_cell_properties()`](style-functions.md) has been renamed
  [`style_cells()`](style-functions.md). It is retained as a
  soft-deprecated alias.

- Various themes have been tweaked:

  - [`theme_basic()`](themes.md) now has bold headers and no header
    column by default.

  - [`theme_plain()`](themes.md) defaults to `position = "centre"`.

  - [`theme_striped()`](themes.md) uses grey stripes, a white border,
    and subtler headers.

  - [`theme_article()`](themes.md) has thinner borders.
...
- You can now use [markdown](https://commonmark.org/help/) within table
  ...
  - Or use the convenience function
    [`set_markdown_contents()`](set_markdown_contents.md) to set cell
...
- Huxtable now has the concept of header row and columns.
  ...
  - To set other rows to be headers, use
    ...
    [`header_cols()`](header_cols.md) or
    [`set_header_cols()`](header_cols.md).

  - New functions [`style_headers()`](style-functions.md),
    [`style_header_cols()`](style-functions.md), and
    [`style_header_rows()`](style-functions.md) to set multiple
    properties on headers.

  - In themes, `header_row/col = TRUE` set the first row/col to a
    header, and style all header rows/cols.

- [`set_bold()`](bold.md) and [`set_italic()`](bold.md) now use a
  default value of `TRUE`. So you can write e.g.
...
- New functions [`fmt_percent()`](fmt_percent.md) and
  [`fmt_pretty()`](fmt_pretty.md) for passing into
  [`number_format()`](number_format.md):

      jams$Sugar <-c ("Sugar content", 0.4, 0.35, 0.45)
      set_number_format(jams, -1, "Sugar", fmt_percent(1))

- [`split_across()`](split-across-down.md) and
  [`split_down()`](split-across-down.md) split a huxtable into a list of
  sub-tables. Headers can be automatically included.

- [`restack_across()`](restack-across-down.md) and
  [`restack_down()`](restack-across-down.md) split a huxtable, then join
  it back up. This is useful for making a table fit on a page.

- [`merge_across()`](merge_across.md) and
  [`merge_down()`](merge_across.md) merge an area of cells horizontally
  across rows, or vertically down columns.

- New functions
  `set_lr_borders()/_border_colors()/_border_styles()/_padding()` set
  left and right borders and padding simultaneously. New functions
  [`set_tb_borders()`](set-multiple.md) etc. set top and bottom
  properties simultaneously. There are `map_` equivalents of all of
  these.

- [`set_outer_padding()`](set-outer.md) sets padding around a range of
  cells, similarly to [`set_outer_borders()`](set-outer.md).

- A new table-level property, [`caption_width()`](caption_width.md),
  allows you to set the width of the caption. The default, `NA`, sets
  the width equal to the table width.

- There are two new themes: [`theme_compact()`](themes.md) and
  [`theme_bright()`](themes.md).

- For [`huxreg()`](huxreg.md), a new function
  [`tidy_replace()`](tidy_override.md) allows you to replace the output
  of `tidy(x)` entirely.
...
- Improvements to [`as_flextable()`](as_flextable.md).

- Improvements to [`quick_pptx()`](quick-output.md) (thanks
  @davidgohel).
...
- New versions of the
  ...
  have an [`as_huxtable()`](as_huxtable.md) method.
...
- [`by_quantiles()`](by_quantiles.md),
  [`by_equal_groups()`](by_quantiles.md) and
  [`by_colorspace()`](by_colorspace.md) have gained a `colwise`
  argument, which calculates quantiles or colors separately for each
  column.

- Add caption support for [`as_flextable()`](as_flextable.md) (thanks
  @sjewo).
...
- New [`merge_repeated_rows()`](merge_repeated_rows.md) function: merge
  repeated rows into a single cell.
...
- Bugfix: text colors in [`hux_logo()`](hux_logo.md).
...
- Add [`quick_latex()`](quick-output.md) function.
...
- New “wrapleft” and “wrapright” options to [`position()`](position.md)
  allow text wrapping around tables.

- New [`set_outer_border_colors()`](set-outer.md) and
  [`set_outer_border_styles()`](set-outer.md) functions, like
  [`set_outer_borders()`](set-outer.md).

- Huxtable no longer requires the `broom` package, instead using the
  `generics` package. If you use [`huxreg()`](huxreg.md), you will still
  need e.g. `broom` or `broom.mixed` to provide
  [`tidy()`](https://generics.r-lib.org/reference/tidy.html) and
  [`glance()`](https://generics.r-lib.org/reference/glance.html) methods
  for specific models.

- Bugfix: [`tidy.tidy_override()`](tidy_override.md) and
  [`glance.tidy_override()`](tidy_override.md) should work even if
  underlying object has no
...
- Bugfix: [`caption()`](caption.md) and [`height()`](height.md) weren’t
  playing nicely.
...
- Enhancement: left- and right-aligned captions are now set above the
  ...
  install this using
  e.g. [`install_latex_dependencies()`](report_latex_dependencies.md) or
  `tlmgr` if it is not already on your system.

- Enhancement: in [`huxtable()`](huxtable.md) and friends,
...
- Argument `byrow` is soft-deprecated. Use [`by_cols()`](by_rows.md)
  instead.
...
- … and a cool new randomized [`hux_logo()`](hux_logo.md) ;-)
...
- The new `autoformat` argument lets [`huxtable()`](huxtable.md) and
  [`as_huxtable()`](as_huxtable.md) automatically choose alignment and
  number format based on column type. Set
