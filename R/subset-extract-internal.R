...
arrange_spans <- function(new_ht,
  ...
  for (i in seq_len(nrow(new_ht))) {
    for (j in seq_len(ncol(new_ht))) {
      ms <- merge_sets[i, j]
...
normalize_index.numeric <- function(idx, max_dim, dim_names) {
  ...
  if (any(idx_oob)) {
    ...
    if (!identical(as.integer(sort(idx[idx_oob])), filled_in_seq)) {
      stop(
        "Missing new rows/columns in subscript: huxtable dimension is ", max_dim,
