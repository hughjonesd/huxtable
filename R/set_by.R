...
#'
#' ```
#' pval_hux %>% set_bold_by(ranges(0.05, c(TRUE, FALSE)))
#' ```
#'
#'  Or you can use red text for a particular value:
#'
#' ```
#' hxtbl %>% set_text_color_by(values("Warning" = "red"))
#' ```
#'
...
#' ```
#'
#' Here, `row` and `col` specify a range of rows and columns. See [rowspecs] for details of
...
#' ```
#'
#' There are functions to set properties in different ways:
#'
#' * To set a property for cells with a specific value, use [values()].
#' * To set a property for cells in a numeric range, use [ranges()].
#' * To set a property for cells by quantiles, use [quantile_ranges()] or [equal_ranges()].
#'
...
#' `values(1:3, letters[1:3])` to see an example.
#'
#' @name set-by
#'
#' @examples
...
#' ht <- set_text_color_by(ht, values(OK = "green", Warning = "orange", Error = "red"))
#' ht
#'
#' ht <- hux(rnorm(5), rnorm(5), rnorm(5))
#' set_background_color_by(ht, ranges(c(-1, 1), c("blue", "yellow", "red")))
#' set_background_color_by(ht, equal_ranges(2, c("red", "green")))
#'
#' ht <- hux(Coef = c(3.5, 2.4, 1.3), Pval = c(0.04, 0.01, 0.07), add_colnames = TRUE)
#' set_bold_by(ht, everywhere, "Pval", ranges(0.05, c(TRUE, FALSE)))
NULL
...
#' @examples
#' ht <- hux(letters[1:3])
#' set_background_color_by(ht, values(a = "red", c = "yellow"))
#' set_background_color_by(ht, values(a = "red", c = "yellow", "green"))
values <- function (...) {
  vals <- c(...)
...
#' @examples
#' ht <- huxtable(c(1, 3, 5))
#' ht <- set_background_color_by(ht, ranges(c(2, 4), c("red", "yellow", "blue")))
#' ht
#' set_background_color_by(ht, ranges(c(2, 4), "pink", extend = FALSE))
#'
#' set_background_color_by(ht, ranges(c(1, 5), c("red", "yellow", "green"), right = TRUE))
#' set_background_color_by(ht, ranges(c(1, 5), c("red", "yellow", "green"), right = FALSE))
ranges <- function (breaks, values, right = FALSE, extend = TRUE) {
...
#' set_background_color_by(ht, quantile_ranges(c(0.2, 0.8), c("red", "white", "green")))
#' set_background_color_by(ht, equal_ranges(3, c("red", "yellow", "green")))
quantile_ranges <- function (quantiles, values, right = TRUE) {
  ...
  qr_fun <- function (ht, rows, cols, current) {
    ...
    rf <- ranges(q_breaks, values, right = right, extend = TRUE)
...
#'
#' @rdname quantile_ranges
#' @export
#' @examples
#' ht <
equal_ranges <- function (n, values) {
  quantile_ranges(seq(1/n, 1 - 1/n, 1/n), values)
}
