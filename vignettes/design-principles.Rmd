...
output:
  ...
  pdf_document:
    keep_tex: yes
vignette: |
  %\VignetteIndexEntry{Design Principles, Comparisons and Limitations} %\VignetteEngine{knitr::rmarkdown} \usepackage[utf8]{inputenc}
---
...
* `texreg` is very good for producing regression tables. It can cope with a huge variety of inputs.
  Hopefully, with the advent of [broom](https://cran.r-project.org/web/packages/broom/vignettes/broom.html), 
...
* You can't change border width - any value above 0 just means there is a border present.
* Table width may be unpredictable and your text may spill over. Adjust on a trial and error basis.
* Left and right cell padding won't work as expected when your text wraps. 
