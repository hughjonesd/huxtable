...
version is available on the web in [HTML](https://hughjonesd.github.io/huxtable/introduction-to-huxtable.html) or 
[PDF](https://hughjonesd.github.io/huxtable/introduction-to-huxtable.pdf) format.
...
column names to the data frame itself, using the `add_colnames` argument to `hux`. We're going to print them out, so they need to be part of the actual table.
...
prop_hux <- hux(as.data.frame(props))                     %>% 
      ...
      set_font(-1, every(), ss_font)                      %>% 
      set_bold(1, every(), TRUE)                          %>% 
      set_width(0.9)                                      %>% 
      set_background_color(every(), evens(), grey(.9))    %>% 
      set_left_border(every(), 1, 1)                      %>% 
      set_right_border(every(), final(), 1)               %>% 
      set_top_border(1, every(), 1)                       %>% 
      set_bottom_border(1, every(), 1)                    %>% 
      set_bottom_border(final(), every(), 1)              %>% 
      set_top_padding(2)                                  %>% 
...
`set_*` functions for cell properties are called like this: `set_xxx(ht, row, col, value)` or like this: `set_xxx(ht,
value)`. If you use the second form, then the value is set for all cells. `set_*` functions for table properties are
always called like `set_xxx(ht, value)`. We'll learn more about this interface in a moment.
...
To select all rows or all columns, use `every()` or `every(1)` in the row or column specification. (Mnemonic: set *every
one* of the rows or columns.) To select just even-numbered rows or columns, use `evens()` or `odds()`. To select the
last `n` rows or columns, use `final(n)`.

With these functions it is easy to add striped backgrounds or outer borders to tables:

```{r}
car_ht %>% 
      set_left_border(every(), 1, 1)           %>%    # left outer border   - every row, first column
      set_right_border(every(), final(1), 1)   %>%    # right outer border  - every row, last column
      set_top_border(1, every(), 1)            %>%    # top outer border    - first row, every column
      set_bottom_border(final(1), every(1), 1) %>%    # bottom outer border - last row, every column
      set_background_color(evens(), every(), 'wheat') # horizontal stripe   - even rows, all columns
```


Of course you could also just do `1:nrow(car_ht)`, but, in the middle of a dplyr pipe, you may not know exactly
how many rows or columns you have. Also, these functions are easy to understand, whereas `seq(2, ncol(ht), 2)` is
a bit complex.
...
`set_*` functions have one more optional argument, the `byrow` argument, which is `FALSE` by default. Use this when you
are setting a pattern of property values for many cells and want to specify a pattern by row:
