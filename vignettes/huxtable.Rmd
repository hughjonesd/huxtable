...
```{r}
car_ht                                                 %>% 
      set_background_color(evens, everywhere, "salmon") %>% 
...
single pattern for many cells, you may want the pattern to fill the matrix by column or by row. The default sets the
pattern going down the columns. If you set `byrow = TRUE`, the pattern goes across rows instead. 
...
For example, let's apply a text color to cars with a lot of horsepower:

```{r}

car_ht %>% set_text_color_by(everywhere, 4, 
  by_range(c(120, 200), c('black', 'red', 'orange'))
)

```

`by_range` is a *mapping function*. It divides its input into ranges and outputs colours based on that, a bit like
the base R function `cut`.

You can also apply properties for cells that match a string, using the function `by_matching`. 
Here, we bold the Mazda cars:

```{r}
car_ht %>% set_bold_by(by_matching('Mazda' = TRUE))
```
