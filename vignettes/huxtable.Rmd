...
car_ht %>% set_text_color_by(everywhere, 4, 
  map_ranges(c(120, 200), c('black', 'red', 'orange'))
)

```

`map_ranges` is a *mapping function*. It divides its input into ranges and outputs colours based on that, a bit like
the base R function `cut`.

You can also apply properties for cells that match a string, using the function `map_matches`:

```{r}
car_ht %>% set_bold_by(map_matches('Mazda' = TRUE))
```
