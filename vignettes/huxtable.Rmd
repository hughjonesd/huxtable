...
* The last line, `set_caption(...)`, sets another *table property*: the 
  table caption.
* Table notes are also table properties. Use `set_table_notes()` to replace
  them or `add_table_note()` to append a note without adding a data row. Use
  `set_cell_note()` for a note referenced from one or more cells. Referenced
  notes are printed after table notes, and `set_note_symbol()` chooses numeric,
  Roman, alphabetic or custom marks. `add_footnote()` is retained for
  compatibility only and is soft-deprecated.
...
* `quick_typst_pdf/png/svg()` create PDFs, PNG images and SVG images using the
  [typst](https://typst.app) system.
