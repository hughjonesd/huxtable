...
* `set_bottom_border(jams, 1, everywhere)` sets the `bottom_border` property.
  ...
  of 0.4.
* So far, all these properties have been *cell properties*. The next line,
  `set_caption(...)`, sets a *table-level* property: the table caption.
...
`names(x)[1] <- "Name"` to change the first name of a vector. Similarly,
in huxtable, you can write `bold(jams)[1, 1:ncol(jams)] <- TRUE` to set the
bold property on the first row of cells.
...
props <- list()
props[["Cell Text"]] <- sort(c("font", "text_color", "wrap", "bold", "italic", "font", "font_size", "na_string", "escape_contents", "markdown", "number_format", "rotation"))
...
props[["Table"]]  <- sort(c("width", "height", "position", "caption", "caption_pos",
...
* Use characters for column names:
...
In `set_` functions, there are some extra shortcuts:
...
There are also some huxtable-specific shortcuts.
...
Each huxtable cell has 12 border properties: 

* `left_border`, `right_border`, `top_border`, `bottom_border` specify the
  border width; 
* `left_border_style`, `right_border_style` and so on specify the border
  style, which can be `"solid"`, `"double"`, `"dotted"`, or `"dashed"`;
* `left_border_color` and so on specify the border color.

You can set these individually:
...
That can be a hassle, so there are some shortcuts. To set width, style and
colour together, use a `brdr()` object:
...
To set all the borders of a cell, use `set_all_borders()`. Here's a corporate 
look for our jams:
...
* Lastly, `set_align()` centres all the cells in the first row.

We might also want to reduce duplication in the "Species" column. We can
use `merge_repeated_rows()` to do this.
...
`merge_repeated_rows()` merges duplicated rows into a single multirow cell.
...
The `restack_across()` function reorganizes our table to fit into fewer rows 
(and more columns). There's a similar `restack_down()` function which fits a 
table into more rows and fewer columns. To understand these, a bit of color will 
help:
...
this with the table-level `width` property, and the *column* property `col_width`.
...
iris_hux_wide %>% 
      ...
      set_font_size(8) %>% 
      set_lr_padding(2) %>% 
      set_col_width(everywhere, rep(c(0.4, 0.2, 0.2, 0.2, 0.2), 3)/3)
```
...
iris_hux %>% 
      ...
      style_headers(bold = TRUE, text_color = "grey20")
```
...
use `style_header_rows()` and `style_header_columns()` to treat header rows
...
iris_hux %>% 
      map_text_color(-(1:2), -1, 
        by_colorspace("red", "grey50", "navy", colwise = TRUE)
      )
...
The syntax of all `map_xxx` functions is `map_property(ht, row, col, fn)`, where
...
You can also apply properties for cells that match a string, using the `by_regex` 
 function:
...
The [NEWS file](https://cran.r-project.org/web/packages/huxtable/news/news.html) lists
