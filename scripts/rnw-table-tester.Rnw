...
\section{Position and width (and caption)}
...
\section{Different kinds of floats}
...
\section{Valign, row height and wrap}
...
\section{Border color left and right/top and bottom}
...
\section{Multirow, multicol and border color}
...
\section{Horizontal border width}
...
\section{Border width and multirow/col}
...
ht2 <- set_left_border(ht2, where(row(ht2) == col(ht2)), 0)
ht2 <- set_right_border(ht2, where(row(ht2) == col(ht2) + 1), 0)

caption(ht2) <- 'Varying borders within rows/cols'
ht2

ht <- set_left_border(ht, 3:7)
ht <- set_bottom_border(ht, 3:7, byrow = TRUE)
