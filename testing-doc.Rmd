...
megahux <- megahux                                %>% 
      ...
      set_rowcol(set_background_color, 9, 'wheat')  %>% 
      set_rowcol(set_text_color, 10, 'red')         %>% 
...
colspan(megahux)[11, 4] <- 6
rowspan(megahux)[4, 11] <- 6
megahux[11,4] <- stringi::stri_rand_lipsum(1)
megahux[4, 11] <- stringi::stri_rand_lipsum(1)
megahux[,16] <- megahux[16,] <- '<b>Non escaped HTML</b>'
megahux[,17] <- megahux[17,] <- sample(c(1:10, rep(NA, 12)))
